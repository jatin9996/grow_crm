...
<tr id="subscription_{{ $subscription->subscription_id }}">
    <td class="subscriptions_col_id">
        <a href="{{ url('') }}/subscriptions/{{ $subscription->subscription_id }}">{{ runtimeSubscriptionIdFormat($subscription->subscription_id) }}
        </a></td>
    @if(config('visibility.subscriptions_col_client'))
    <td class="subscriptions_col_company">
        <a
            href="{{ url('') }}/clients/{{ $subscription->client_id }}">{{ str_limit($subscription->client_company_name ?? '---', 12) }}</a>
    </td>
    @endif
    <td class="subscriptions_col_plan">
        <a
            href="{{ url('') }}/subscriptions/{{ $subscription->subscription_id }}">{{ str_limit($subscription->subscription_gateway_product_name ?? '---', 25) }}</a>
    </td>
    ...
    <td class="subscriptions_col_action actions_column">
        ...
        <span class="list-table-action dropdown font-size-inherit">
            ...
            <a href="{{ url('') }}/subscriptions/{{ $subscription->subscription_id }}" title="{{ cleanLang(__('lang.view')) }}"
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <div class="dropdown-menu" aria-labelledby="listTableAction">
                    ...
                    <a class="dropdown-item"
                        href="{{ url('') }}/subscriptions/{{ $subscription->subscription_id }}">
