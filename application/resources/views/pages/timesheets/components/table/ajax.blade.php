...
<tr id="timesheet_{{ $timesheet->timer_id }}">
    ...
    <td class="timesheets_col_related">
        ...
        <a
            href="{{ url('') }}/projects/{{ $timesheet->timer_projectid }}">{{ str_limit($timesheet->project_title ?? '---', 25) }}</a>
        @else
        <a href="{{ url('') }}/leads/{{ $timesheet->timer_leadid }}">{{ str_limit($timesheet->lead_title ?? '---', 25) }}</a>
        @endif
    </td>
    @endif
