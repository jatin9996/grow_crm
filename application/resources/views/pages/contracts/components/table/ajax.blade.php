...
< @count($contracts); $i++) <!--each row-->
    <tr id="contracts_{{ $contracts[$i]['contract_id'] ?? '' }}">
        ...
        <td class="contracts_col_title">
            <a
                href="{{ url('') }}/contracts/{{ $contracts[$i]['contract_id'] ?? '' }}">{{ str_limit('New Laravel Based Website Project This Is Great', 30) }}</a>
        </td>
        <td class="contracts_col_contract">
            <a
                href="{{ url('') }}/contracts/{{ $contracts[$i]['contract_id'] ?? '' }}">{{ str_limit('Microsoft Private Limited Company', 20) }}</a>
        </td>
        ...
        <td class="contracts_col_action actions_column">
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                @endif
                <a href="{{ url('') }}/contracts/{{ $contract->contract_id ?? '' }}" title="{{ cleanLang(__('lang.view')) }}" class="data-toggle-action-tooltip btn btn-outline-info btn-circle btn-sm">
