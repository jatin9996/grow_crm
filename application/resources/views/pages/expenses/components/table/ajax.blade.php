...
<tr id="expense_{{ $expense->expense_id }}">
    ...
    <td class="expenses_col_client">
        <a
            href="{{ url('') }}/clients/{{ $expense->expense_clientid }}">{{ str_limit($expense->client_company_name ?? '---', 12) }}</a>
    </td>
    @endif
    <!--column visibility-->
    @if(config('visibility.expenses_col_project'))
    <td class="expenses_col_project">
        <a href="{{ url('') }}/projects/{{ $expense->expense_projectid }}">{{ str_limit($expense->project_title ?? '---', 12) }}</a>
    </td>
    @endif
